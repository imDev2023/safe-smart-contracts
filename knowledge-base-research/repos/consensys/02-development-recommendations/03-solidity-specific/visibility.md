...
- `Private` functions and state variables are only visible for the contract they are defined in and not in derived contracts. **Note**: [Everything inside a contract is visible to all observers external to the blockchain](https://solidity.readthedocs.io/en/develop/contracts.html?#visibility-and-getters), even `Private` variables.
