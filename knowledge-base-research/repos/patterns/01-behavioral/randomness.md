...
**Source:** https://fravoll.github.io/solidity-patterns/randomness.html
...
*   **Oracle RNG** - randomness provided by an oracle, see [Oracle pattern](https://fravoll.github.io/solidity-patterns/oracle.html)
*   **Collaborative PRNG** - collaborative generation of a random number within the blockchain

Because the use of an oracle has already been discussed in the [respective pattern](https://fravoll.github.io/solidity-patterns/oracle.html) and the most renown example of collaborative PRNG, [Randao](https://github.com/randao/randao), is not being actively developed anymore, we will focus on the generation of pseudorandom numbers with the help of block hashes in this chapter. Considerations between the use of oracle RNG versus block hash PRNG will be discussed in the Consequences section.
...
To get rid of the possibility of interference by miners and prediction of random numbers, Bonneau et al. proposed a solution applied on the Bitcoin blockchain: a trusted party provides a seed, which will be hashed together with a future block hash, to make it impossible for the miner to predict the outcome of his block hash on the random number. We are using this idea in this pattern to avoid interference by malicious miners.
